...
class TwitchClient(AbstractClient):
    ...
    async def connect(self, channel: str) -> None:
        self.reader, self.writer = await asyncio.open_connection(self.TWITCH_IRC_HOST, self.TWITCH_IRC_PORT)

        self.writer.write(f"PASS anonymous\r\n".encode('utf-8'))
        ...
        try:
            while self.reader and not self.reader.at_eof():
                ...
                if len(parts) > 2 and parts[1] == "PRIVMSG":
                    ...
                    await self.message_callback(author, content)
